...
abstract class BaseActivity<B : ViewBinding>(val bindingFactory: (LayoutInflater) -> B) : LocalizationActivity(), CoroutineScope {
    val TAG = "~~~"
    ...
    open fun binding() {
        if (isFullScreen) window.setFlags(
    ...
    protected open fun initLanguage() {
        if (AppPreferences.Companion.instance.isChooseLanguage) {
            val locale = Locale(AppPreferences.Companion.instance.currentLanguage)
    ...
    override fun dispatchTouchEvent(ev: MotionEvent): Boolean {
        ...
        return if (ev.action == MotionEvent.ACTION_DOWN) {
            if (ConsentDialogManager.Companion.instance?.showConsentDialogOnButtonClick(this) == true) {
                true
            } else {
