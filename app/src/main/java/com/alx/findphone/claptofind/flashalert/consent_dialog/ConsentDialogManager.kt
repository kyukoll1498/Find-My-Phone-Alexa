package com.alx.findphone.claptofind.flashalert.consent_dialog

import android.app.Activity
import android.content.Intent
import androidx.appcompat.app.AppCompatActivity
import com.alx.findphone.claptofind.flashalert.consent_dialog.base.BaseDialogConsentManager
import com.alx.findphone.claptofind.flashalert.consent_dialog.dialog.BillingDialog
...
class ConsentDialogManager : BaseDialogConsentManager() {
    ...
    fun showConsentDialogSplash(
        ...
        RemoteConfigManager.Companion.instance!!.loadIsShowConsent(
            activity!!,
                ...
                if (key == Constants.KEY_CANCEL) {
                    val dismissedListener =
                        ConsentForm.OnConsentFormDismissedListener { error: FormError? ->
                            RemoteConfigManager.Companion.instance!!.loadReshowGDPRSplashCount(
    ...
    fun showConsentDialogOnButtonClick(activity: Activity): Boolean {
        return if (RemoteConfigManager.Companion.instance!!.isShowConsent) {
            if (!consentRequired(activity)) {
                return false
            }
            if (canRequestAds(activity)) {
    ...
    private fun showOnButtonClick(
        ...
        buttonClickCount++
        return if (buttonClickCount == RemoteConfigManager.Companion.instance!!.limitFunctionClickCount()) {
            BillingDialog.getInstance()!!.setCallback { key: String, data: Array<Any?>? ->
                if (key == Constants.KEY_CANCEL) {
